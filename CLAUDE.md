...
- **Vite 8** (exact-pinned `8.2.0`, `rolldown` exact-pinned `1.2.3` via `overrides`) for the lib build, upgraded 2026-08. The classic `manualChunks` function API silently mis-chunks `src/core/*` singletons under Rolldown's default `codeSplitting` (recursive dependency capture — see the CRITICAL comment in `vite.config.ts`); fixed via `output.codeSplitting.groups` + `includeDependenciesRecursively: false`. That fix also requires `featureChunk()` to explicitly route every file under a feature's directory to that feature's chunk (not just its `index.ts`) — with recursive capture off, unclassified helper files (e.g. `hover/effects.ts`, `tabs/state.ts`) no longer get swept in implicitly and land as separate, colliding auto-named chunks otherwise. This is a mid-deprecation-cycle Rolldown API with open upstream issues (vitejs/vite#21271, rolldown/rolldown#7114, rolldown/rolldown#2118) — exact-pinned, not `^`, so a routine install/update can't silently reintroduce the bug. `scripts/verify-chunk-structure.mjs` (wired into `npm run build`) is the safety net — it must pass before either pin moves.
